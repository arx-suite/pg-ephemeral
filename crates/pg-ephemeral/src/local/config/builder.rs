...
impl LocalBuilder {
    ...
    pub fn build(self) -> LocalBuilderResult<LocalConfig> {
        // database port
    ...
    #[must_use]
    #[inline]
    fn allocate_port(&self) -> LocalBuilderResult<u16> {
    ...
    #[must_use]
    #[inline]
    fn temp_dir(&self) -> LocalBuilderResult<TempDir> {
    ...
    #[must_use]
    fn bin_base_path(&self) -> LocalBuilderResult<PathBuf> {
        ...
        let bin_base_path = match self.bin_base_path.clone() {
            Some(base_path) => {
                if !std::fs::exists(&base_path)? {
                    return Err(LocalBuilderError::BinaryPathNotExists(base_path))?;
                }
